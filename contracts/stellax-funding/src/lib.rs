...
use stellax_math::{
    ...
    MAX_FUNDING_RATE_PER_HOUR, TTL_BUMP_INSTANCE, TTL_BUMP_PERSISTENT, TTL_THRESHOLD_INSTANCE,
...
impl StellaxFunding {
    ...
    pub fn update_funding(env: Env, market_id: u32) -> Result<(), FundingError> {
        ...
        let hourly_rate = current_funding_rate(&env, &cfg, &market)?;
        let accumulated_delta = prorated_funding_delta(hourly_rate, elapsed)?;
        ...
        state.last_funding_rate = hourly_rate;
        write_state(&env, market_id, &state);

        env.events().publish(
            (symbol_short!("fundupd"), market_id),
            (hourly_rate, elapsed, state.accumulated_funding_long),
        );
        Ok(())
    }
    ...
    pub fn get_current_funding_rate(env: Env, market_id: u32) -> Result<i128, FundingError> {
        ...
        let cfg = read_config(&env)?;
        let market = read_market(&env, market_id)?;
        current_funding_rate(&env, &cfg, &market)
    }
...
fn prorated_funding_delta(hourly_rate: i128, elapsed_seconds: u64) -> Result<i128, FundingError> {
    hourly_rate
        ...
        .and_then(|value| value.checked_div(3600))
        .ok_or(FundingError::MathOverflow)
}
